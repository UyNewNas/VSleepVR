...
The Tauri-to-UI integrity boundary mirrors that backend window selection without re-running runtime-state classification. For current reports with an unambiguous `session_id`, it derives the first/last parseable authoritative-observation timestamps, applies every trustworthy one-sided recording edge, and requires `observed_window_ms` to match the resulting span. `invalid_order` deliberately ignores both contradictory edges and uses the full authoritative-observation range, matching `timeline.rs`; `missing_both` and fully ambiguous boundaries likewise fall back to that range. Inferred, malformed, and wrong-source forensic rows cannot move a fallback endpoint. If there is no parseable authoritative timed evidence, the expected window is `null`.
